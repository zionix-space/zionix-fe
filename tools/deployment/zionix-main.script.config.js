      ...
      apps.forEach((app) => {
        ...
        if (fs.existsSync(projectJsonPath)) {
          try {
            ...
            console.warn(`⚠️  Warning: Could not read project.json for ${app}, using folder name`);
            allModules.push({
              name: app,
              domain: domain,
              app: app,
          ...
          console.warn(`⚠️  Warning: project.json not found for ${app}, using folder name`);
          allModules.push({
            name: app,
            domain: domain,
            app: app,
  ...
  return new Promise((resolve, reject) => {
    ...
    const retryCheck = () => {
      ...
      if (retries >= maxRetries) {
        reject(new Error(`Remote ${remoteName} failed to start after ${maxRetries} attempts`));
        return;
      }
      
      console.log(`⏳ Waiting for remote ${remoteName}... (attempt ${retries}/${maxRetries})`);
...
function getRemotePort(remoteName) {
  ...
  console.log(`⚠️  No port found in project.json for ${remoteName}, using generated port`);
