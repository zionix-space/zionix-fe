...
import type {ComponentData} from '@zionix-formEngine/core'
import {
  array,
  boolean,
  ComponentStore,
...
// Key differences:
// - Steps component is similar but uses different prop names
// - Button and ButtonToolbar replaced with Button and Space
// - StepItemProps replaced with StepProps
...
const RsWizard = ({
  ...
  const getStepStatus = useCallback((index: number): StepProps['status'] => {
  ...
  const buttons = useMemo(() => (
    <Space style={toolbarStyle} className={'buttons'}>
  ...
  // Map wizard items to Ant Design Steps items format
  const stepsItems = useMemo(() => {
    return wizardItems.map(({label, onClick, status, className}) => ({
      title: showStepsLabels ? label : undefined,
      status,
      className,
      onClick
    }))
  }, [wizardItems, showStepsLabels])
