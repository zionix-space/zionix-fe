...
export const reorderChildren = (children, splitDropZonePath, splitItemPath) => {
  ...
  if (splitDropZonePath.length === 1) {
    ...
    return reorder(safeChildren, itemIndex, dropZoneIndex);
  }

  const updatedChildren = [...safeChildren];
...
export const removeChildFromChildren = (children, splitItemPath) => {
  ...
  if (splitItemPath.length === 1) {
    const itemIndex = Number(splitItemPath[0]);
    return remove(safeChildren, itemIndex);
  }
...
export const addChildToChildren = (children, splitDropZonePath, item) => {
  ...
  if (splitDropZonePath.length === 1) {
    const dropZoneIndex = Number(splitDropZonePath[0]);
    return insert(safeChildren, dropZoneIndex, item);
  }
  ...
  switch (splitDropZonePath.length) {
    case 1: {
      ...
      if (item.type === COLUMN) {
        ...
        // moving row to section level - use directly without wrapping
