...
const MenuForm = ({ selectedKey, selectedItem, allMenuKeys, menuData, onChange, onDelete, onAddChild, onRefetch, isDeleting }) => {
    ...
    useEffect(() => {
        ...
        if (selectedKey && selectedKey !== previousSelectedKeyRef.current) {
            ...
            if (selectedItem) {
                ...
                // Force form reset first to clear all fields
                form.resetFields();
    ...
    const handleSaveClick = async () => {
        try {
            ...
            // Helper function to find menu item by ID (menu_id, application_id, module_id, or _tempId)
            const findItemById = (items, id) => {
                for (const item of items) {
                    const itemId = item.menu_id || item.application_id || item.module_id || item._tempId;
            ...
            // Get application_id from the root menu (first level in tree - Application)
            ...
            // Get module_id based on the level:
            // - Level 2 (Module itself): Use the item's own module_id if updating, or it will be generated by backend if creating
            // - Level 3+ (Menus): Traverse up the parent path to find the module (level 2)
            let moduleId = null;

            if (formValues.level === 2) {
                // This IS a module (level 2)
                // For updates, use the existing module_id from selectedItem
                // For creates, backend will generate it, so we don't send it
                if (!isNewItem && selectedItem.module_id) {
                    moduleId = selectedItem.module_id;
                }
            } else if (formValues.level >= 3) {
                // This is a menu (level 3+), need to find the parent module's module_id
                // The module is always at level 2, which is parentPath[1]
                // But we need to traverse the entire parent path to find it

                // Method 1: Check parentPath[1] (should be the module)
                if (parentPath.length >= 2) {
                    const moduleMenuId = parentPath[1]; // Second level is the module
                    const moduleMenu = findItemById(menuData.mainNavigation, moduleMenuId);
                    if (moduleMenu && moduleMenu.module_id) {
                        moduleId = moduleMenu.module_id;
                    }
                }

                // Method 2: If not found, traverse up the parent path to find any item with module_id
                if (!moduleId) {
                    for (const pathId of parentPath) {
                        const pathItem = findItemById(menuData.mainNavigation, pathId);
                        if (pathItem && pathItem.module_id && !pathItem.menu_id) {
                            // Found a module (has module_id but no menu_id)
                            moduleId = pathItem.module_id;
                            break;
                        }
                    }
                }

                // Method 3: If still not found, check the direct parent's module_id
                if (!moduleId && parentMenu && parentMenu.module_id) {
                    moduleId = parentMenu.module_id;
                }

                // Method 4: If still not found, check the selected item itself (for updates)
                if (!moduleId && !isNewItem && selectedItem.module_id) {
                    moduleId = selectedItem.module_id;
                }
            }
            ...
            // Determine parent_menu_id based on hierarchy:
            // - Level 1 (Application): parent_menu_id = null
            // - Level 2 (Module): parent_menu_id = null (direct child of application)
            // - Level 3+ (Menus): parent_menu_id = parent's menu_id
            let parentMenuId = null;
            if (parentMenu) {
                // Check if parent is the application (level 1: has application_id but no menu_id/module_id)
                const isApplication = parentMenu.application_id && !parentMenu.menu_id && !parentMenu.module_id;

                // Check if parent is a module (level 2: has module_id but no menu_id)
                const isModule = parentMenu.module_id && !parentMenu.menu_id;

                if (isApplication || isModule) {
                    // Parent is application or module, so parent_menu_id = null
                    parentMenuId = null;
                } else {
                    // Parent is a menu (level 3+), use its menu_id
            ...
            // Add module_id for level 2+ (modules and menus)
            // Level 2: module_id is the item's own ID (only for updates, creates will get it from backend)
            // Level 3+: module_id is the parent module's ID
            if (moduleId) {
            ...
            <BaseForm form={form} layout="vertical" onValuesChange={handleFieldChange} className="menu-form-content menu-editor-scrollbar" size="middle">
                ...
                <div className="menu-form-section" style={{ borderBottom: `1px solid ${token.colorBorderSecondary}` }}>
                    ...
                    <div className="menu-form-grid-row-2">
                        ...
                        <BaseForm.Item label="Module ID" name="module_id" tooltip="Read-only system field - Required for level 3+ menus">
                            <BaseInput placeholder="System managed" disabled />
                        </BaseForm.Item>
